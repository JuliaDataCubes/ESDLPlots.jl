...
#   xaxis::NamedTuple{(:index, :axis)}
#   yaxis::NamedTuple{(:index, :axis)}
#   rgbaxis::NamedTuple{(:index, :axis)}
#   dmin
#   dmax
#   c_1::Int
#   c_2::Int
...
# get_ranges(p::MAPPlotRGB,d::Int, axlist, iCube) = (d==p.ilon || d==p.ilat) ? 1:length(axlist[d]) :
#                                                   d==p.irgb ? axVal2Index(axlist[d],p.chan[iCube],fuzzy=true) :
#                                                   axVal2Index(axlist[d],v_d,fuzzy=true)
...
# end
#
# plotCall(p::MAPPlotCategory) = p.im_only ?
#   :(_makeMaprgb(a_1,m_1,0.0,0.0,colorm,oceancol,misscol,legPos,iscategorical,false,[])[5]) :
...
# plotCall(p::MAPPlotContin)   = p.im_only ?
# :(_makeMaprgb(a_1,m_1,mi,ma,colorm,oceancol,misscol,legPos,iscategorical,symmetric, tickspos)[5]) :
...
#   end
#
#   ixax = findAxis(xaxis,c)
#   iyax = findAxis(yaxis,c)
#   xax  = (index=ixax, axis=axlist[ixax])
#   yax  = (index=iyax, axis=axlist[iyax])
#   rgbaxis = (index=irgb, axis=axlist[irgb])
#
